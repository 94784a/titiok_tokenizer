#####################
# Copied from https://github.com/FoundationVision/InfinityStar/blob/main/infinity/models/videovae/modules/quantizer/vector_quantize_pytorch.py
#####################
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        reinmax: bool = False,  # using reinmax for improved straight-through, assuming straight through helps at all
        sync_codebook: bool = None,
        ...
        sync_update_v: float = 0.0,  # the v that controls optimistic vs pessimistic update for synchronous update rule (21) https://minyoungg.github.io/vqtorch/assets/draft_050523.pdf
    ):
        """

        Args:
            dim (int): 输入特征维度
            codebook_size (int): 码本中code的个数, 如8192
            codebook_dim (int, optional): 码本每个向量的维度, 如果为None则等于dim. Defaults to None.
            heads (int, optional): 多头VQ, 把输入拆成多个头进行量化. Defaults to 1.
                如果heads > 1, 则内部会把x reshape成[heads, B, N, D_head]去量化.
            separate_codebook_per_head (bool, optional): 是否多头共享一个码本. Defaults to False.
            decay (float, optional): EMA更新码本时的decay, 越接近1更新越慢(越稳定). Defaults to 0.8.
            eps (float, optional): 数值稳定用. Defaults to 1e-5.
            kmeans_init (bool, optional): 启用mini-batch K-means来初始化码本而不是随机init. Defaults to False.
            kmeans_iters (int, optional): K-means迭代数. Defaults to 10.
            sync_kmeans (bool, optional): DDP时是否做同步K-means (各卡一起算). Defaults to True.
            use_cosine_sim (bool, optional): 是否用余弦相似度为距离(CosineSimCodebook), 否则是欧式距离(EuclideanCodebook). Defaults to False.
                余弦适合做 feature-level 量化（对范数不敏感），欧式更常见，也支持 affine 参数的 trick.
            threshold_ema_dead_code (float, optional): 死码本阈值, 如果某个code使用次数小于这个阈值, 会被重置为新的样本. Defaults to 0.
            channel_last (bool, optional): channel在最后一维. Defaults to True.
            accept_image_fmap (bool, optional): 如果是True,输入是[B, C, T, H, W]特征,内部做reshape. Defaults to False.
            commitment_weight (float, optional): VQ 里标准的 commitment loss 权重. Defaults to 1.
            commitment_use_cross_entropy_loss (bool, optional): commitment loss是否用交叉熵. Defaults to False.
                - 如果 False commitment loss 用 MSE ||sg[z] - e||^2.
                - 如果 True, commitment loss 用交叉熵, 对 distances 做 CE, 这个是给类似 residual VQ / NS2
                    那种“想让编码拉向正确 index 但不用 hard assignment”的玩法。
            orthogonal_reg_weight (float, optional): 对码本做正交正则(减少不同 code 之间相关性). Defaults to 0.
                0 表示开启, 会在训练阶段额外加一个损失
            orthogonal_reg_active_codes_only (bool, optional): 只对当前batch被激活的codes做正交正则, 可以减少无用的码本更新. Defaults to False.
            orthogonal_reg_max_codes (int, optional): 限制参与正交正则的最多 code 数, 防止 codebook 太大时计算量爆炸. Defaults to None.
            stochastic_sample_codes (bool, optional): 量化时会启用 Gumbel-Softmax 的 stochastic sampling而不是argmax. Defaults to False.
            sample_codebook_temp (float, optional): Gumbel Softmax 的温度,温度越高越平滑,越低越接近 one-hot. Defaults to 1.
            straight_through (bool, optional): 开启 straight-through estimator, 前向是离散 code, 反向梯度直接从 soft version 传. Defaults to False.
            reinmax (bool, optional): 一个改进版 straight-through(Reinmax), 理论上二阶性质更好,对 Gumbel-ST 做了一些修正. Defaults to False.
                注意reinmax 必须配合 straight_through=True.
            sync_codebook (bool, optional): 是否在DDP下同步码本. Defaults to None.
                如果不传, 会自动看distributed.is_initialized()
            sync_affine_param (bool, optional): DDP下是否同步affine的统计量. Defaults to False.
            ema_update (bool, optional): 是否启用EMA更新码本. Defaults to True.
            learnable_codebook (bool, optional): 码本可学习, 与ema_update互斥. Defaults to False.
                - 如果 True, 码本是nn.Parameter, 通过反向传播 + 优化器来训练, 而不是EMA. 更接近普通embedding
                - 如果 False, EMA VQ
            in_place_codebook_optimizer (Callable[..., Optimizer], optional): 原地码本优化器. Defaults to None.
                如果给一个优化器函数, 在forward中会用当前batch的quantize和x做一个MSE loss, 直接loss.backward()一步更新码本(不会影响主网络的optimizer).
            affine_param (bool, optional): 额外维护codebook均值方差, batch均值方差. 在quantize时, 对输入和codebook做对齐,让量化更稳定.
            affine_param_batch_decay (float, optional): 控制这些均值方差的 EMA 更新速率. Defaults to 0.99.
            affine_param_codebook_decay (float, optional): 控制这些均值方差的 EMA 更新速率. Defaults to 0.9.
            sync_update_v (_type_, optional): 在ST基础上,再加一个(quantize - quantize.detach())形态的修正项. Defaults to 0.
    ...
    def get_perplexity(self, encoding_indices, x):
        encode_onehot = F.one_hot(encoding_indices, self.codebook_size).type_as(
            x
        )  # [bthw, ncode]
    ...
    def get_usage(self, encoding_indices):
        ...
        codebook_usage_percentage = torch.zeros(
            self.codebook_size, device=all_indices.device
        )
    ...
    def forward(
        ...
        if self.accept_image_fmap:
            embed_ind = rearrange(
                embed_ind, "b (t h w) ... -> b t h w ...", t=nframes, h=height, w=width
            )
        ...
        if self.accept_image_fmap:
            quantize = rearrange(
                quantize, "b (t h w) c -> b c t h w", t=nframes, h=height, w=width
            )
        ...
        if self.call_cnt == 0:
            ...
            self.codebook_usage.data = (
                0.99 * self.codebook_usage.data + (1 - 0.99) * usage
            )

        self.call_cnt += 1
        # avg_distribution = self.codebook_usage.data.sum() / self.codebook_size
        avg_usage = (
            self.codebook_usage.data > (1 / self.codebook_size)
        ).sum() / self.codebook_size
